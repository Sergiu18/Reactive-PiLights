...
app.get('/api/lights_off', cors(), (req, res) => {
	...
	res.send({message: "Lights turned off", color: `Red: 0, Green: 0, Blue: 0`}, state())
});
...
app.get('/api/modes_off', cors(), (req, res) => {
	...
	lightController.stroboscopic_off();
	lightController.rainbow_off();
	res.send({message: "Modes turned off"}, state())
});
...
app.get('/api/setColor', cors(), (req, res) => {
	...
	if(r || g || b){
		...
		{
			res.status(500);
			res.send({message: "All the parameters must be numbers within 0 - 255"}, state())
			return
		}
	...
	res.status(500);
	res.send({message: `Invalid colors: Red: ${r}, Green: ${g}, Blue: ${b}`}, state())
});
...
app.get('/api/toggleStroboscopic', cors(), (req, res) => {
	...
	{
		lightController.stroboscopic_off();
		res.send({message: "Stroboscopic-off"}, state())
		...
		if((red || green || blue) && lightController.state.rainbow==false)
		{
			...
			res.send({message: "Stroboscopic-on"}, state())
		}	
		else
		{
			res.status(500);
			res.send({message: "Please select colors or close other modes before performing this action!"}, state())
		}
});
...
app.get('/api/toggleRainbow', cors(), (req, res) => {
	...
	{
		lightController.rainbow_off();
		res.send({message: "Rainbow_off"}, state())
	}
	else
	{
		if(lightController.state.stroboscop==false)
		{
			...
			res.send({message: "Rainbow-on"}, state())
			console.log("toggleRainbow");
		}
		else
			res.send({message: "Please close other modes before performing this action!"} , state())
	}
});
